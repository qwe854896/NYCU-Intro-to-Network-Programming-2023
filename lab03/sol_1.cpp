...
void read_maze(SocketStream &sock_stream, vector<vector<int>> &maze, int &start_x, int &start_y, int &end_x, int &end_y)
{
    char _;
...
int dfs(SocketStream &sock_stream, vector<vector<int>> &maze, int x, int y, const int &ex, int ey, string &steps)
{
    if (maze[x][y] == Exit)
        return 4;
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (abs(ex - nx) >= abs(ex - x) && abs(ey - ny) >= abs(ey - y))
            continue;

        if (maze[nx][ny] != Wall)
        {
            int ret = dfs(sock_stream, maze, nx, ny, ex, ey, steps);
            if (ret != -1)
            {
                steps += D[i];
                return i;
...
int main()
{
    int sockfd = connect_to_server("140.113.213.213", 10301);
    if (sockfd == -1)
    {
        return 1;
    }
    ...
    int start_x, start_y;
    int end_x, end_y;
    read_maze(sock_stream, maze, start_x, start_y, end_x, end_y);

    sock_stream.getline(_);
    sock_stream.getline(_);

    string steps = "";
    dfs(sock_stream, maze, start_x, start_y, end_x, end_y, steps);
